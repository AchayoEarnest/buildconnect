            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500">
                  <span className="flex items-center gap-1"><MapPin className="w-4 h-4" />{engineer.location_city}, {engineer.location_country}</span>
                  <span className="flex items-center gap-1"><Briefcase className="w-4 h-4" />{engineer.years_exp} yrs exp</span>
                  {engineer.website_url && (
                    <a href={engineer.website_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-brand-600 hover:underline">
                      <Globe className="w-4 h-4" />Website
                    </a>
                  )}
                </div>
              </div>
              <ContactButton engineerSlug={engineer.slug} engineerId={engineer.id.toString()} />
            </div>
            <div className="flex flex-wrap items-center gap-4 mt-4">
              <StarRating value={engineer.avg_rating} />
              <span className="text-sm text-gray-500">({engineer.review_count} reviews)</span>
              <span className={`text-xs px-3 py-1 rounded-full font-medium capitalize ${availabilityColor[engineer.availability]}`}>{engineer.availability}</span>
            </div>
            <div className="flex flex-wrap gap-2 mt-4">
              {engineer.skills.map((skill) => (
                <span key={skill.id} className="text-xs px-3 py-1 rounded-full bg-brand-50 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300 font-medium">{skill.name}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <aside className="space-y-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800 sticky top-6">
            ...
            <ContactButton engineerSlug={engineer.slug} engineerId={engineer.id.toString()} className="w-full" variant="primary" />
          </div>
